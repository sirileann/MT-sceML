...
from src.model import PointCloudTransformer_PCT, PointCloudTransformer_SPCT
from src.model import MultiPCT_BackPC_BackDM, MultiPCT_Back_Fix, MultiPCT_Back_ESL_Fix
...
class PCT_Cls(pl.LightningModule):
    def __init__(self, outputNr: int = 30, submodel: str = "pct", run_name: str = "default",
                 loss_func: torch.nn.Module = nn.MSELoss(), pkl_path: str = "original_dataset.pkl", pretrained=False,
                 ckpt_path='', only_last_layer=False):
        super().__init__()
        ...
        if submodel == "pct":
            self.model = PointCloudTransformer_PCT(outputNr)
            if "original1024" in self.pkl_path:
                self.example_input_array = torch.Tensor(1, 1024, 3)
            elif "original2048" in self.pkl_path:
                self.example_input_array = torch.Tensor(1, 2048, 3)
            elif "original4096" in self.pkl_path:
                self.example_input_array = torch.Tensor(1, 4096, 3)
            elif "original8192" in self.pkl_path:
                self.example_input_array = torch.Tensor(1, 8192, 3)
            elif "italian1024" in self.pkl_path:
                self.example_input_array = torch.Tensor(1, 1024, 3)
            else:
                print("Unknown number of points in input point clouds")

        elif submodel == "fixPoints":
            self.model = PointCloudTransformer_SPCT(outputNr)
            self.example_input_array = torch.Tensor(1, 12, 3)

        elif submodel == "backPC_backDM":
            self.model = MultiPCT_BackPC_BackDM(outputNr)
            self.example_input_array = torch.Tensor(1, 1024, 3), torch.Tensor(1, 1, 640, 480)

        elif submodel == "back_fix":
            self.model = MultiPCT_Back_Fix(outputNr)
            self.example_input_array = torch.Tensor(1, 1024, 3), torch.Tensor(1, 12, 3)

        elif submodel == "back_esl_fix":
            self.model = MultiPCT_Back_ESL_Fix(outputNr)
            self.example_input_array = torch.Tensor(1, 1024, 3), torch.Tensor(1, 12, 3), torch.Tensor(1, 60, 3)

        else:
            raise ValueError(
                'No valid submodel chosen. Choose between {"pct", "fixPoints", "backPC_backDM", "back_fix", "back_esl_fix"}')
        ...
        if only_last_layer:
            ...
            for param in self.model.cls.linear3.parameters():
    ...
    def forward(self, x, y=None, z=None):
        if self.submodel in ["pct", "fixPoints"]:
            return self.model(x)

        elif self.submodel in ["backPC_backDM", "back_fix"]:
    ...
    def training_step(self, batch, batch_idx):
        if self.submodel in ["pct", "fixPoints"]:
            ...
            pcds, images, targets, path_id = batch
            batch_size = pcds.size(0)

            # Forward pass
            outputs = self.forward(pcds, images)
            loss = self.loss_func(outputs, targets)
        else:
            pcd, pcd2, pcd3, targets, path_id = batch
            batch_size = pcd.size(0)

            # Forward pass
            outputs = self.forward(pcd, pcd2, pcd3)
            loss = self.loss_func(outputs, targets)
    ...
    def test_step(self, batch, batch_idx):
        ...
        reshaped_outputs = outputs.view(2, num_points).t()
        reshaped_targets = targets.view(2, num_points).t()
        ...
        if "original1024" in self.pkl_path:
            ...
            path_back = path_back.replace("./preprocessed", "./data/preprocessed_italian1024")
            path_fixPoints = path_fixPoints.replace("./preprocessed", "./data/preprocessed_italian1024")
        ...
        array1[:, 1] = np.linspace(np.asarray(corrected_fixPoints['fix_C7'][1]),
                                   np.asarray(corrected_fixPoints['fix_DM'][1]), num_points)
        array1[:, 2] = targets_np[0, num_points:]
        array2[:, 0] = outputs_np[0, :num_points]
        array2[:, 1] = np.linspace(np.asarray(corrected_fixPoints['fix_C7'][1]),
                                   np.asarray(corrected_fixPoints['fix_DM'][1]), num_points)
        ...
        # reduction = getattr(self.loss_func, 'reduction')
        ...
        fixPoints = go.Scatter3d(
            x=corrected_fixPoints_np[:, 0],
            y=corrected_fixPoints_np[:, 1],
            z=corrected_fixPoints_np[:, 2],
            mode='markers',
            name='fixPoints',
            marker=dict(
                size=2,
        ...
        # plot the figure
        fig = go.Figure(data=lines + [red_line, green_line, red_dots, green_dots, back, fixPoints], layout=layout)
        ...
        fig.write_html(
            f"./logs/{self.logger.name}/version_{str(self.logger.version)}/plotlys/subject_{path_id}_acc_{meanl2norm:.2f}.html")
