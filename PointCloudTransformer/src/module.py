...
from utils.util import sample_and_knn_group
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        attention_scores = torch.matmul(projected_query, projected_key.transpose(-2, -1))
        attention_weights = self.softmax(attention_scores)

        # Apply attention weights to the values
        output = torch.matmul(attention_weights, projected_value)
...
class Embedding(nn.Module):
    ...
    def forward(self, x):
        ...
        Output
            x: [B, out_channels, N]
        """
        x = x.permute(0, 2, 1)  # added to have correct input order
