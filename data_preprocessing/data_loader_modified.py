...
def preprocess_and_save_data(df: pd.DataFrame, num_points: int = 8192, italian=False) -> (
    ...
    if italian:
        ...
        df_cleaned.to_pickle(f"italian{str(num_points)}_dataset_August_cleaned.pkl")
    else:
...
def random_augment_pcd(df: pd.DataFrame, rot_limit: float = 0, rot_angle: float = 3):
    ...
    df.to_pickle("augmented1024_dataset_italian.pkl")
...
def calculate_mean_spine(dir_isl: str, show: bool = False):
    isl_files = sorted([f for f in os.listdir(dir_isl) if f.endswith(".npy")])
    ...
    for file in isl_files:
        path_id = int(file.split('_')[-1].split('.')[0])
        path_ids.append(path_id)

        isl_data = np.load(os.path.join(dir_isl, file))
        if isl_data.shape != (60, 1):
            isl_data = isl_data.reshape(-1, 1)

        x_points = np.squeeze(isl_data[:30])
        y_points = np.linspace(0, 1, 30).transpose()
        z_points = np.squeeze(isl_data[30:])
        isl = np.column_stack((x_points, y_points, z_points))
        all_isls.append(isl)

    # Convert the list of arrays into a single 3D array
    all_isls_array = np.array(all_isls)
    all_paths = np.array(path_ids)

    # Calculate the average spine
    average_spine = np.mean(all_isls_array, axis=0)
    ...
    if show:
        ...
        # Plot individual spines
        for spine in all_isls_array:
            ax.plot(spine[:, 0], spine[:, 1], spine[:, 2], color='blue', alpha=0.5)

        # Plot average spine
        ax.plot(average_spine[:, 0], average_spine[:, 1], average_spine[:, 2], color='red', linewidth=2)
    ...
    return average_spine, all_isls_array, all_paths


def convert_and_plot(np1, np2):
    x_points = np.squeeze(np1[:30])
    y_points = np.linspace(0, 1, 30).transpose()
    z_points = np.squeeze(np1[30:])
    isl1 = np.column_stack((x_points, y_points, z_points))

    np2 = np2.reshape(-1, 1)
    x_points = np.squeeze(np2[:30])
    y_points = np.linspace(0, 1, 30).transpose()
    z_points = np.squeeze(np2[30:])
    isl2 = np.column_stack((x_points, y_points, z_points))
...
if __name__ == "__main__":
    ...
    # random_augment_pcd(df_cleaned)
    # prepare_dataset(pd.read_pickle("augmented1024_dataset_italian.pkl"), stop=len(pd.read_pickle("augmented1024_dataset_italian.pkl")))

    # # to clean invalid scans from italian dataset uncomment this line
    # clean_italian("raw_data/Italian_data_August2023", "raw_data/evaluation_results.txt")

    # # to create a new (italian) dataset uncomment this line
    # df, df_cleaned = create_dataset_italian("./raw_data/Italian_data_cleaned_August2023", num_points=1024)
    df_cleaned = pd.read_pickle("italian1024_dataset.pkl")
    print("")
    ...
    # with open("raw_data/Formetric_Croatian_data_July2023/croatian_7/7_metadata.json", "r") as f:
    #     data = json.load(f)
    #     fixPoints_dict = data["fixPts"]
    # fixPoints_np = np.array(list(fixPoints_dict.values()))
    # display_pcd_plotly(fixPoints_np)

    # back = convert_ply_to_np("raw_data/Formetric_Croatian_data_July2023/croatian_1/1_backscan.ply")
    # for i in [7, 29, 59, 131, 217, 409, 424, 440]:
    # back = np.load(f"preprocessed_original1024/backscan/backscan_pp_{274}.npy")
    # isl = np.load("preprocessed_original1024/isl/isl_pp_274.npy")
    # display_pcd_plotly(back, isl)

    # back = "raw_data/Formetric_Croatian_data_August2023/croatian_1/1_backscan.ply"
    #
    # point_cloud = o3d.io.read_point_cloud(back)  # Replace with your point cloud file path
    # o3d.visualization.draw_geometries([point_cloud])

    # import os
    #
    # # Example usage
    # # convert_and_plot(np.load("preprocessed_italian1024/isl_30/isl_30_70.npy"),
    # #                  np.load("spines_predicted/isl_predicted_70.npy"))
    # average_spine_italian, _, _ = calculate_mean_spine("preprocessed_italian1024/isl_30")
    # average_spine_prediction, all_isls_prediction, _ = calculate_mean_spine("spines_predicted")
    # average_spine_prediction_cnn, all_isls_prediction_cnn, _ = calculate_mean_spine("spines_predicted_cnn")
    # average_spine_prediction_bal, all_isls_prediction_bal, _ = calculate_mean_spine("spines_predicted_bal")
    # average_spine_target, all_isls_target, path_ids = calculate_mean_spine("spines_target")
    #
    # df = pd.read_pickle("italian1024_dataset.pkl")
    # print("Mean Scaling Factor", df.scaling_factor.mean())
    #
    # from scipy.spatial.distance import cdist
    #
    # scale_f = df.scaling_factor.mean()
    # average_spine_italian = average_spine_italian / scale_f
    # average_spine_prediction = average_spine_prediction / scale_f
    #
    ...
    # sagittal_distances = []
    #
    # for spine_pred in all_isls_prediction:
    #     spine_pred = spine_pred / scale_f
    #     mean_distance = np.mean(np.sqrt(abs(spine_pred[:, 0] - average_spine_italian[:, 0]) ** 2 + abs(
    #         spine_pred[:, 2] - average_spine_italian[:, 2]) ** 2))
    #     mean_distances.append(mean_distance)
    #
    #     front_dist = np.mean(np.sqrt(abs(spine_pred[:, 0] - average_spine_italian[:, 0]) ** 2))
    #     front_distances.append(front_dist)
    #
    #     sagittal_dist = np.mean(np.sqrt(abs(spine_pred[:, 2] - average_spine_italian[:, 2]) ** 2))
    #     sagittal_distances.append(sagittal_dist)
    #
    # mean_distances = np.array(mean_distances)
    # front_distances = np.array(front_distances)
    # sagittal_distances = np.array(sagittal_distances)
    #
    # print("Mean L2-Norm (Av. Spine Italian, Prediction Spines)", np.mean(mean_distances))
    # print("Standard Deviation", np.std(mean_distances))
    # print("Mean L2-Norm (Av. Spine Italian, Prediction Spines) Frontal", np.mean(front_distances))
    # print("Standard Deviation", np.std(front_distances))
    # print("Mean L2-Norm (Av. Spine Italian, Prediction Spines) Sagittal", np.mean(sagittal_distances))
    # print("Standard Deviation", np.std(sagittal_distances))
    #
    # distances = np.sqrt(abs(average_spine_prediction[:, 0] - average_spine_italian[:, 0]) ** 2 + abs(
    #     average_spine_prediction[:, 2] - average_spine_italian[:, 2]) ** 2)
    # print("Mean L2 Distance (Av. Spine Italian, Av. Spine Prediction):", np.mean(distances))
    #
    # import matplotlib.pyplot as plt
    #
    # # Plotting
    ...
    # for spine_pred in all_isls_prediction:
    #     spine_pred = spine_pred / scale_f
    #     ax.plot(spine_pred[:, 0], spine_pred[:, 1], spine_pred[:, 2], color='blue', alpha=0.5)
    # ax.plot(average_spine_italian[:, 0], average_spine_italian[:, 1], average_spine_italian[:, 2],
    #         color='red', alpha=0.5, linewidth=4, label="Mean ISL Italian Dataset")
    #
    # ax.set_xlabel('X')
    # ax.set_ylabel('Y')
    # ax.set_zlabel('Z')
    ...
    # # Comparison with average spine
    # for idx in range(len(path_ids)):
    #
    #     spine_pred = all_isls_prediction[idx] / scale_f
    #     spine_targ = all_isls_target[idx] / scale_f
    #
    #     l2_meanItalian_target = np.mean(np.sqrt(abs(spine_targ[:, 0] - average_spine_italian[:, 0]) ** 2 + abs(
    #         spine_targ[:, 2] - average_spine_italian[:, 2]) ** 2))
    #     l2_pred_target = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2 + abs(
    #         spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     l2_meanItalian_target_frontal = np.mean(np.sqrt(abs(spine_targ[:, 0] - average_spine_italian[:, 0]) ** 2))
    #     l2_pred_target_frontal = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2))
    #
    #     l2_meanItalian_target_sagittal = np.mean(np.sqrt(abs(spine_targ[:, 2] - average_spine_italian[:, 2]) ** 2))
    #     l2_pred_target_sagittal = np.mean(np.sqrt(abs(spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     # Create the figure and subplots
    #     fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 10))
    #
    #     # First plot: X-Y plane
    #     ax1.plot(average_spine_italian[:, 0], average_spine_italian[:, 1],
    #              color='blue', label="Mean ISL X-Ray")
    #     ax1.plot(spine_targ[:, 0], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax1.plot(spine_pred[:, 0], spine_pred[:, 1], color='green', label="Predicted Spine")
    #
    #     ax1.set_xlim(-150, 150)
    #     ax1.set_xlabel('X')
    #     ax1.set_ylabel('Y')
    #     ax1.set_title(f'Frontal Plane', fontsize=14)
    #     ax1.set_aspect('equal')
    #
    #     ax1.text(0.5, -0.1, f"Mean L2-Error (Mean ISL X-Ray, Target ISL): {l2_meanItalian_target_frontal:.2f} mm\nMean L2-Error (Predicted ISL, Target ISL): {l2_pred_target_frontal:.2f} mm", horizontalalignment='center',
    #              verticalalignment='center', transform=ax1.transAxes, fontsize=12)
    #
    #     # Second plot: Y-Z plane
    #     ax2.plot(average_spine_italian[:, 2], average_spine_italian[:, 1],
    #              color='blue', label="Mean ISL X-Ray")
    #     ax2.plot(spine_targ[:, 2], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax2.plot(spine_pred[:, 2], spine_pred[:, 1], color='green', label="Predicted Spine")
    #
    #     ax2.set_xlim(-150, 150)
    #     ax2.set_xlabel('Z')
    #     ax2.set_ylabel('Y')
    #     ax2.set_title('Sagittal Plane', fontsize=14)
    #     ax2.set_aspect('equal')
    #
    #     ax2.text(0.5, -0.1,
    #              f"Mean L2-Error (Mean ISL X-Ray, Target ISL): {l2_meanItalian_target_sagittal:.2f} mm\nMean L2-Error (Predicted ISL, Target ISL): {l2_pred_target_sagittal:.2f} mm",
    ...
    #     fig.legend(lines, labels, loc='lower center', ncol=3, fontsize=14)
    #
    #     # Add an overall title to the figure
    #     plt.suptitle(f'Subject ID {path_ids[idx]}\nGlobal Mean L2-Error (Mean ISL X-Ray, Target ISL): {l2_meanItalian_target:.2f} mm\nGlobal Mean L2-Error (Predicted ISL, Target ISL): {l2_pred_target:.2f} mm', fontsize=16)
    #
    #     # Adjust layout for the legend
    #     plt.subplots_adjust(bottom=0.15)
    #
    #     # Save and close the figure
    #     plt.savefig(f"plots/av_spine/subject_{path_ids[idx]}.pdf")
    #     plt.savefig(f"plots/av_spine/subject_{path_ids[idx]}.png")
    #     plt.close()

    # Comparison CNN with PCT
    # for idx in range(len(path_ids)):
    #     spine_pred = all_isls_prediction[idx] / scale_f
    #     spine_pred_cnn = all_isls_prediction_cnn[idx] / scale_f
    #     spine_targ = all_isls_target[idx] / scale_f
    #
    #     l2_pred_target = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2 + abs(
    #         spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #     l2_pred_target_cnn = np.mean(np.sqrt(abs(spine_pred_cnn[:, 0] - spine_targ[:, 0]) ** 2 + abs(
    #         spine_pred_cnn[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     l2_pred_target_frontal = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2))
    #     l2_pred_target_frontal_cnn = np.mean(np.sqrt(abs(spine_pred_cnn[:, 0] - spine_targ[:, 0]) ** 2))
    #
    #     l2_pred_target_sagittal = np.mean(np.sqrt(abs(spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #     l2_pred_target_sagittal_cnn = np.mean(np.sqrt(abs(spine_pred_cnn[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     # Create the figure and subplots
    #     fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 10))
    #
    ...
    #
    #     ax1.plot(spine_targ[:, 0], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax1.plot(spine_pred[:, 0], spine_pred[:, 1], color='green', label="Predicted Spine - PCT")
    #     ax1.plot(spine_pred_cnn[:, 0], spine_pred_cnn[:, 1],
    #              color='lawngreen', label="Predicted Spine - CNN")
    #
    #     ax1.set_xlim(-150, 150)
    #     ax1.set_xlabel('X')
    #     ax1.set_ylabel('Y')
    ...
    #
    #     ax1.text(0.5, -0.1,
    #              f"CNN - Mean L2-Error: {l2_pred_target_frontal_cnn:.2f} mm\nPCT - Mean L2-Error: {l2_pred_target_frontal:.2f} mm",
    #              horizontalalignment='center',
    #              verticalalignment='center', transform=ax1.transAxes, fontsize=12)
    #
    #     # Second plot: Y-Z plane
    #
    #     ax2.plot(spine_targ[:, 2], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax2.plot(spine_pred[:, 2], spine_pred[:, 1], color='green', label="Predicted Spine - PCT")
    #     ax2.plot(spine_pred_cnn[:, 2], spine_pred_cnn[:, 1],
    #              color='lawngreen', label="Predicted Spine - CNN")
    #
    #     ax2.set_xlim(-150, 150)
    #     ax2.set_xlabel('Z')
    ...
    #
    #     ax2.text(0.5, -0.1,
    #              f"CNN - Mean L2-Error: {l2_pred_target_sagittal_cnn:.2f} mm\nPCT - Mean L2-Error: {l2_pred_target_sagittal:.2f} mm",
    #              horizontalalignment='center',
    #              verticalalignment='center', transform=ax2.transAxes, fontsize=12)
    #
    #     # Combine legends
    ...
    #     # Add an overall title to the figure
    #     plt.suptitle(
    #         f'Subject ID {path_ids[idx]}\nCNN - Global Mean L2-Error: {l2_pred_target_cnn:.2f} mm\nPCT - Global Mean L2-Error: {l2_pred_target:.2f} mm',
    #         fontsize=16)
    #
    #     # Adjust layout for the legend
    #     plt.subplots_adjust(bottom=0.15)
    #
    #     # Save and close the figure
    #     plt.savefig(f"plots/cnn_pct/subject_{path_ids[idx]}.pdf")
    #     plt.savefig(f"plots/cnn_pct/subject_{path_ids[idx]}.png")
    #     plt.close()

    # # Comparison PCT with PCT Balanced
    # for idx in range(len(path_ids)):
    #     spine_pred = all_isls_prediction[idx] / scale_f
    #     spine_pred_bal = all_isls_prediction_bal[idx] / scale_f
    #     spine_targ = all_isls_target[idx] / scale_f
    #
    #     l2_pred_target = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2 + abs(
    #         spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #     l2_pred_target_bal = np.mean(np.sqrt(abs(spine_pred_bal[:, 0] - spine_targ[:, 0]) ** 2 + abs(
    #         spine_pred_bal[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     l2_pred_target_frontal = np.mean(np.sqrt(abs(spine_pred[:, 0] - spine_targ[:, 0]) ** 2))
    #     l2_pred_target_frontal_bal = np.mean(np.sqrt(abs(spine_pred_bal[:, 0] - spine_targ[:, 0]) ** 2))
    #
    #     l2_pred_target_sagittal = np.mean(np.sqrt(abs(spine_pred[:, 2] - spine_targ[:, 2]) ** 2))
    #     l2_pred_target_sagittal_bal = np.mean(np.sqrt(abs(spine_pred_bal[:, 2] - spine_targ[:, 2]) ** 2))
    #
    #     # Create the figure and subplots
    #     fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 10))
    #
    ...
    #     ax1.plot(spine_targ[:, 0], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax1.plot(spine_pred[:, 0], spine_pred[:, 1], color='green', label="Predicted Spine - PCT")
    #     ax1.plot(spine_pred_bal[:, 0], spine_pred_bal[:, 1],
    #              color='lawngreen', label="Predicted Spine - PCT Balanced")
    #
    #     ax1.set_xlim(-150, 150)
    #     ax1.set_xlabel('X')
    #     ax1.set_ylabel('Y')
    ...
    #
    #     ax1.text(0.5, -0.1,
    #              f"PCT - Mean L2-Error: {l2_pred_target_frontal:.2f} mm\nPCT Balanced - Mean L2-Error: {l2_pred_target_frontal_bal:.2f} mm",
    #              horizontalalignment='center',
    #              verticalalignment='center', transform=ax1.transAxes, fontsize=12)
    #
    #     # Second plot: Y-Z plane
    ...
    #     ax2.plot(spine_targ[:, 2], spine_targ[:, 1], color='red', label="Target Spine")
    #     ax2.plot(spine_pred[:, 2], spine_pred[:, 1], color='green', label="Predicted Spine - PCT")
    #     ax2.plot(spine_pred_bal[:, 2], spine_pred_bal[:, 1],
    #              color='lawngreen', label="Predicted Spine - CNN")
    #
    #     ax2.set_xlim(-150, 150)
    #     ax2.set_xlabel('Z')
    ...
    #
    #     ax2.text(0.5, -0.1,
    #              f"PCT - Mean L2-Error: {l2_pred_target_sagittal:.2f} mm\nPCT Balanced - Mean L2-Error: {l2_pred_target_sagittal_bal:.2f} mm",
    #              horizontalalignment='center',
    #              verticalalignment='center', transform=ax2.transAxes, fontsize=12)
    #
    #     # Combine legends
    ...
    #     # Add an overall title to the figure
    #     plt.suptitle(
    #         f'Subject ID {path_ids[idx]}\nPCT - Global Mean L2-Error: {l2_pred_target:.2f} mm\nPCT Balanced - Global Mean L2-Error: {l2_pred_target_bal:.2f} mm',
    #         fontsize=16)
    #
    #     # Adjust layout for the legend
    #     plt.subplots_adjust(bottom=0.15)
    #
    #     # Save and close the figure
    #     plt.savefig(f"plots/pct_bal/subject_{path_ids[idx]}.pdf")
    #     plt.savefig(f"plots/pct_bal/subject_{path_ids[idx]}.png")
    #     plt.close()
